...
class VMLibraryWindowController: NSWindowController, NSTableViewDataSource, NSTableViewDelegate, NSWindowDelegate {
    ...
    private func downloadAndStartMacOSVM(_ vmConfig: VMConfiguration) {
    ...
    private func showNewVMDialog() {
        ...
        if alert.runModal() == .alertFirstButtonReturn {
            ...
            do {
                ...
                if needsISO {
                    if osType == "macOS" {
                        // For macOS, download IPSW automatically
                        downloadAndStartMacOSVM(newVM)
                    } else {
