...
#   3. Install CSIRT monitoring: DTrace scripts, ESF helper, log streaming
...
#   (See: csirt-vf-build-base.sh step 3 for the recovery boot procedure.)
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
# ─────────────────────────────────────────────────────────────────────────────
# 4. CSIRT MONITORING — macOS NATIVE STACK
# These integrate with your existing CSIRT-VF monitoring infrastructure.
# The macOS guest uses ESF + DTrace instead of the Linux auditd/tcpdump.
# ─────────────────────────────────────────────────────────────────────────────
log "Installing CSIRT monitoring stack (macOS native)"
...
# ── CSIRT telemetry LaunchDaemon ───────────────────────────────────────────────
# Runs DTrace continuously, appends JSON lines to workspace telemetry dir
sudo tee /Library/LaunchDaemons/com.secvf.ai-sandbox.csirt-execmon.plist << 'PLIST'
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>Label</key>
    <string>com.secvf.ai-sandbox.csirt-execmon</string>
    ...
    <array>
        ...
        <string>-o</string>
        <string>/workspace/.csirt-telemetry/dtrace-exec.jsonl</string>
    </array>
    <key>RunAtLoad</key>
    <true/>
    <key>KeepAlive</key>
    ...
    <string>/var/log/secvf-ai-sandbox-csirt-execmon.err</string>
</dict>
</plist>
PLIST
...
# Create CSIRT telemetry directories (Change 4: known paths for sentinel)
sudo mkdir -p "${WORKSPACE_MOUNT}/.csirt-telemetry/kali-netcap"
sudo chown -R "${AI_SANDBOX_USER}:staff" "${WORKSPACE_MOUNT}/.csirt-telemetry"
sudo chmod -R 750 "${WORKSPACE_MOUNT}/.csirt-telemetry"
...
sudo launchctl load /Library/LaunchDaemons/com.secvf.ai-sandbox.csirt-execmon.plist  2>/dev/null || true
